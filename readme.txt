...
8. Manipulating date & time in an object-oriented way i.e. using "DateTime" class: In 24 hours format, midnight is represented as 00:00:00 and in 12 hours format, midnight is represented by 12:00:00AM. H => 24 hours format, h => 12 hours format.
   ...
   Following is same as above, but in procedural way:
...
9. Case Study:
   	...
   	c. Yes, it's right, date() function's 2nd parameter must be integer.
		...
		//Output: 4 => 32 bit PHP, 8 => 64 bit PHP
      	Difference between Bits & Bytes (Bits vs Bytes): https://youtu.be/SHfc5yHINE4 : A simple, non-technical 3-4 minutes explanation.
      	Why 1 byte = 8 bit or 9 bit or 10 bit; and why a character size in JAVA is 2 byte: https://youtu.be/R8woTMB7npI : Here explained the smallest unit of data-storage, history of way of doing old-programming on computer up to the current time, how a single character like "A" or "b" are stored in computer and how it's size might be 8 bit or 9 bit or 10 bit based on code-system used.
      	Size of int variable 2 or 4 bytes: https://youtu.be/s0fvKsp1Qwo : Here explained various computer generation storage history i.e. 16 bit, 32 bit & now 64 bit computers. Why old generation computer/software stored a single integer with 2 byte and now modern computer stores a single integer with 4 byte.  	
